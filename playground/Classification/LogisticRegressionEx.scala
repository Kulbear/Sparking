...
val trainingData = (timeData.select(data("Clicked on Ad").as("label"),
...
val Array(training, test) = trainingData.randomSplit(Array(0.7, 0.3), seed = 12345)
...
val testPredictionAndLabel = testResults.select($"prediction", $"label")

val predictionAndLabels = testPredictionAndLabel.as[(Double, Double)].rdd
